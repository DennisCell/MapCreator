            ...
            if (location.type === 'SIMPLE') {
                marker = L.marker([location.latitude, location.longitude], {
                    icon: L.divIcon({
                        ...
                        html: \`<span style="background: #333; color: white; padding: 2px 5px; border-radius: 4px; font-size: 10px;">\${location.name}</span>\`,
                        iconSize: [100, 20],
                        iconAnchor: [50, 10]
                    })
                });
                ...
                if (location.style.icon === 'FLAG') iconHtml = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M14.778.085A.5.5 0 0 1 15 .5V8a.5.5 0 0 1-.314.464L14.5 8l.186.464A.5.5 0 0 1 14.5 9H14v7a.5.5 0 0 1-.5.5H9.5a.5.5 0 0 1 0-1H13V9H2L1.5 8.5l.5-.5H2v-1h1.5v-1H2v-1h1.5V4H2v-1h1.5V2H2V1h1.5V0h1v1h1.5v1H6V1h1.5v1H9V1h1.5v1H12V1h1.5V0h.278zM14 1.422l-2.158 1.573-.01.006-2.16 1.573-2.158 1.573-2.16 1.573L3.398 8H14zM2 9.5V15h1V9.5z"/></svg>';
                
                const shapeRadius = location.style.shape === 'SQUARE' ? '4px' : '50%';
                const markerHtml = \`<div style="width: 28px; height: 28px; background-color: \${location.style.color}; border-radius: \${shapeRadius}; display: flex; align-items: center; justify-content: center; color: white; border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.4);">\${iconHtml}</div>\`;
                ...
                let tooltipHtml = \`<div style="padding: 8px; font-family: sans-serif; background-color: rgba(255,255,255,0.85); border-radius: 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.2); min-width: 200px;">\`;
                tooltipHtml += \`<h2 style="font-size: 16px; font-weight: 700; color: #15803d; margin: 0 0 4px 0; word-wrap: break-word;">\${location.name}</h2>\`;

                if (regularFields.length > 0) {
                    tooltipHtml += \`<div style="margin-top: 4px; padding-top: 4px; border-top: 1px solid #e5e7eb; font-size: 14px;">\`;
                    regularFields.forEach(field => {
                        ...
                        const valueStyle = isDeficitField && valueIsPositive ? 'color: #16a34a; font-weight: 700;' : 'color: #1f2937;';

                        tooltipHtml += \`<div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 2px;"><span style="color: #4b5563; margin-right: 8px;">\${field.name}</span><span style="font-weight: 500; text-align: right; \${valueStyle}">\${value}</span></div>\`;
                    });
                    tooltipHtml += \`</div>\`;
                }

                if (descriptionField && location.customData[descriptionField.id]) {
                    tooltipHtml += \`<div style="padding-top: 8px; margin-top: 8px; border-top: 1px solid #e5e7eb;">\`;
                    tooltipHtml += \`<p style="font-size: 14px; color: #1f2937; white-space: pre-wrap; word-wrap: break-word; margin: 0;">\${location.customData[descriptionField.id]}</p>\`;
                    tooltipHtml += \`</div>\`;
                }
                tooltipHtml += \`</div>\`;
